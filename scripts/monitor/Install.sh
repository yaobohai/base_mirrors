...
Processes=$(ps -ef|grep -v grep|grep -w '/usr/sbin/zabbix_agentd'|wc -l)
if [[ $Processes != '0' ]];then echo "${hostname} Agent process already exists in the system!!!";exit 1;fi
}
...
function Install_5() {
    ...
    if [[ $? != '0' ]];then echo "${hostname} Install failed!!!";fi
...
function AGENT_RUN() {
  ...
  if [[ $STATUS -eq 1 ]];then echo "${hostname} Install OK!!!";else echo "${hostname} Install failed!!!";fi
}
...
if [[ $OS_Version == 5 ]]; then
  ...
  echo "${hostname} ERROR!Unsupported system version..."
fi
}
